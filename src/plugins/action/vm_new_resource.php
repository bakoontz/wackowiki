...
if(@$_POST['_action'] === 'vm_new_resource') {
    ...
    $verses_tag = "Verses";
    $full_tag = Ut::join_path($root_tag, $resource_tag, $verses_tag);
    if(!$this->load_page($full_tag)) {
        // Create new page hierarchy
        // $body = "{{tree depth=2 title=0}}";
        if(null == $this->save_page($full_tag, " ")) {
            $this->set_message($this->_t("Page save failed: ") . $full_tag, 'error');
            $this->reload_me();
        }
        $full_tag = Ut::join_path($root_tag, $resource_tag);
        $body = "{{tree depth=1 title=0}}";
        if(null == $this->save_page($full_tag, $body)) {
            $this->set_message($this->_t("Page save failed: ") . $full_tag, 'error');
            $this->reload_me();
        }
    }
    $this->set_message("Redirecting: " . $this->href('', $tag));
    $this->http->redirect($this->href('', $full_tag));
} else {
