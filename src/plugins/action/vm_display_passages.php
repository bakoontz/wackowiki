...
if(@$_POST['_action'] === 'vm_new_resource') {
    $root_tag = "Resources";
    $resource_tag = $_POST['title'] . " - " . $_POST['author_first'] . " " . $_POST['author_last'];
    if($error = $this->sanitize_new_page_tag($resource_tag)) {
            $this->set_message($error, 'error');
            $this->reload_me();
    }
    $verses_tag = "Verses";
    $full_verses_tag = Ut::join_path($root_tag, $resource_tag, $verses_tag);
    if(!$this->load_page($full_verses_tag)) {
        // Create new page hierarchy
        // $body = "{{tree depth=2 title=0}}";
        if(null == $this->save_page($full_verses_tag, " ")) {
            $this->set_message($this->_t("Page save failed: ") . $full_verses_tag, 'error');
            $this->reload_me();
        }
        $full_resource_tag = Ut::join_path($root_tag, $resource_tag);
        $body = "{{tree depth=1 title=0}}";
        if(null == $this->save_page($full_resource_tag, $body)) {
            $this->set_message($this->_t("Page save failed: ") . $full_resource_tag, 'error');
            $this->reload_me();
        }
    }
    // Create DB record
    $sql = 'INSERT INTO ' . 'vm_resource SET ' .
        'page_id          = ' . $this->get_page_id($full_verses_tag) . ', ' .
    ...
    $this->http->redirect($this->href('', $full_resource_tag));
} else {
